...
Crittercam: fully local wildlife detection, recording, and logging running on a Jetson Orin Nano (this machine). Currently at milestone 1 (live detection boxes + heartbeat); tracking, events, and clip recording are milestone 2, TensorRT backend is milestone 4 — `NotImplementedError` branches in `capture.py`/`detector.py` mark the seams where those land.
...
1. **Tracker** (`crittercam/tracker/`) — capture → detect → publish pipeline. A capture thread feeds a `maxsize=2` queue with drop-oldest semantics (`put_latest`) so the pipeline never falls behind real time; the main loop runs YOLO every `detector.infer_every_n` frames (CPU inference is ~1–3 FPS) and reuses the last detections in between, publishing annotated JPEGs every frame.
...
- `FramePublisher.publish` returns the encoded JPEG so the future recorder can reuse it without a second encode — keep that contract.
